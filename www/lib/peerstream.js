 ...
 if(data.event == 'media-chunk'){
   ...
   if(data.payload != ""){
    ...
    // PeerStream._sendData('media-chunk-ack',"");
       ...
       function win(writer) {
           ...
           writer.onwriteend = function(e) {
               ...
               if (PeerStream.isStartPlay) {
                   ...
                   //  PeerStream.audio_player = new Media(PeerStream._currentBufferFile.localURL, playStopped, playError);
                   
                   var tmpFilePath = cordova.file.tempDirectory+"song.mp3";
                   tmpFilePath = tmpFilePath.replace('file://', '');
                   PeerStream.audio_player = new Media(tmpFilePath, playStopped, playError);
                   //PeerStream.audio_player = new Media(cordova.file.tempDirectory+"song.mp3", playStopped, playError);
                   
                   //PeerStream.audio_player = new Media("/var/containers/Bundle/Application/889F9D0E-EE47-4657-9E9D-E2E987E771D3/liveplay.app/Aayakalaigal - TamilWire.Com.mp3", playStopped,playError);
                   PeerStream.play();
               }
           };
       ...
       window.resolveLocalFileSystemURL(cordova.file.tempDirectory, gotDir, fail);
